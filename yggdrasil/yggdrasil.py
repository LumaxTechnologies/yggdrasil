...
from yggdrasil.data import available_infra_services, provider_secrets_requirements, supported_providers

LIBRARIES_PATH = pkg_resources.resource_filename('yggdrasil', 'libraries')


class Operation:
    """
    A class to store information and data about a Yggdrasil operation.
    ...
    project_root: str
        the root folder ("gitops") for executing Yggdrasil
    scope: str
    ...
    standard_config: dict
        a dictionary of standard VMs and network configurations, merged from Yggdrasil
    ...
    def scope_setup(self, no_config=False):
        ...
        yggdrasil_standard_file = os.path.join(
            self.libraries_path, "internal", "standard", "vm_standard.yml")
        yggdrasil_standard_config = load_yaml(self.logger, yggdrasil_standard_file)
        ...
        self.standard_config = merge_dictionaries(yggdrasil_standard_config, local_standard_config)
    ...
    def secrets_setup(self):
        ...
        for env_var in required_vars + optional_vars:
            if env_var in list(os.environ.keys()):
                self.logger.debug("Using Yggdrasil variable %s from environment" % env_var)
        ...
        if os.path.isfile(self.terraform_output):
            with open(self.terraform_output, 'r') as f:
                try :
                    ...
                    self.logger.critical("Yggdrasil failed to load the Terraform output.\
\nPlease check the content of the file %s" % self.terraform_output)
    ...
    def load_ips(self):
        ...
        if os.path.exists(scope_ips):
            ...
            self.logger.critical("Missing config_ips.yml file, please run yggdrasil <SCOPE> init 1")
            sys.exit()
