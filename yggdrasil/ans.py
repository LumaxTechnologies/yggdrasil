"""Ansible operations for Yggdrasil."""
import os
import string
import sys
import copy
...
from yggdrasil.common_tools import bash_action, j2, load_yaml
...
def load_ssh_parameters_meta(operation: Operation, keep_default_ssh=False):
    ...
    """ this function computes the parameters for the ssh.cfg file when Yggdrasil
...
def create_inventory(operation: Operation):
    ...
    """ this function is the entry function for the 'create_inventory' Yggdrasil command.
...
def install_ansible_dependencies(operation: Operation):
    ...
    by merging the default one from Yggdrasil sources and the one in
...
def prepare_ansible(operation: Operation, securize=False):
    ...
    :param operation: Operation, the current Operation
    :param securize: bool, set to True if we are using the 'yggdrasil ... init Z' command
    ...
    yggdrasil_secret_pattern = re.escape("__CT_SECRET_START__") + "(.*?)" + re.escape("__CT_SECRET_STOP__")

    execute_ansible_content = ""
    with open(execute_ansible_output, "r") as file:
        execute_ansible_content = file.read()

    yggdrasil_ansible_secrets = re.findall(yggdrasil_secret_pattern, execute_ansible_content)
    # get unique secret patterns
    yggdrasil_ansible_secrets = list(set(yggdrasil_ansible_secrets))

    for ct_ansible_secret in yggdrasil_ansible_secrets:
...
def execute_ansible(operation: Operation):
    ...
    if operation.ssh_with_password:
        if "CLOUDTIGER_SSH_PASSWORD" not in ansible_environ.keys():
            ...
            yggdrasil_ssh_password = getpass.getpass(query_string)
            ansible_environ["CLOUDTIGER_SSH_PASSWORD"] = base64.b64encode(
                bytes(yggdrasil_ssh_password, 'utf-8'))
