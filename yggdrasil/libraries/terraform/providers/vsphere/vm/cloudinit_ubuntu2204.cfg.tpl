...
- path: /root/temporary
  ...
  content: |
    ...
    [domain/${domain_ldap}]
    ldap_user_search_base = ${ldap_user_search_base}%{ for ad_group in ad_groups ~}?DC=yggdrasil,DC=tech?subtree?(memberOf=CN=${ad_group},OU=admin_groups,DC=yggdrasil,DC=tech)%{ endfor ~}?DC=yggdrasil,DC=tech?subtree?(memberOf=CN=acl-ssh-${vm_name},OU=admin_groups,DC=yggdrasil,DC=tech)
