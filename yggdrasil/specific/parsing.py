...
class FortiNetworking():
    ...
    Methods
    -------
    get_yggdrasil_format(self, resource_name, resource)
        extract data at Yggdrasil format
    clean_data(key, data, array_params = [])
        reformat data if needed
    """

    def get_yggdrasil_format(self, resource_name, resource):
        resourceYggdrasilFormat = {}
        if resource_name in [
                            'firewall addrgrp',
                            'firewall address',
